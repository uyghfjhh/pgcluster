...
本仓库的 `pgcluster.yaml` 是可直接运行的 `logical1` 示例，默认使用本机 PostgreSQL 18.3：

```bash
./pgcluster validate logical1
./pgcluster doctor logical1
./pgcluster create logical1
./pgcluster status logical1
./pgcluster verify logical1
```
...
然后写入 `1, 'logical1-probe'`，并确认该行从 `pub1` 逻辑复制到 `sub1`，再物理复制到 `sub2`。

默认端口和 PGDATA：

| 节点 | 端口 | 默认 PGDATA |
| --- | ---: | --- |
| pub1 | 15432 | `data/logical1/pub1` |
| pub2 | 15433 | `data/logical1/pub2` |
| sub1 | 25432 | `data/logical1/sub1` |
| sub2 | 25433 | `data/logical1/sub2` |
