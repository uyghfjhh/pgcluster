...
`tui` 是实时监控界面，默认每 3 秒刷新一次。主视图显示部署依赖图、集群健康卡片和实例明细；
顶部 KPI 显示连接使用率、TPS、共享缓存命中率和最大 WAL 延迟。流复制显示 streaming 备库数与
WAL 延迟，逻辑复制显示 subscription 状态和最新 LSN，Citus 显示活跃 Worker，MMR 显示活跃成员。
